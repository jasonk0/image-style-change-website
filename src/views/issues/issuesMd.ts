...
在\`tsconfig.json\`里可以配置别名，\`baseUrl:"."\`配置基础路径，也就是根目录；\`"paths":{"@*" : ["src/*]}\`配置别名，

意为“所有「@」开头的路径匹配为「src/」开头”，但是提示是有了，也可以找到，

但是 vite 并不能找到对应的 module，所以还需要配置 vite，使用 path 包，配置就可以了
...
详细代码见 \`src/util/process.ts\`
使用 canvas 画的一个颜色渐变进度条，使用的是网上找的代码，然后自己修改备注了一下
